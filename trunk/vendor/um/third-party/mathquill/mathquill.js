...
var MathCommand = P(MathElement, function(_, _super) {
  ...
  */
  ...
  _.html = function() {
    ...
    //             * skip the matching top-level close tags and all tags pairs
    //               in between
    // - for each block content marker,
    //     + replace it with the contents of the corresponding block,
    //       rendered as HTML
    //     + insert the mathquill-block-id attribute into the containing tags
    ...
    //   matching top-level open tags, all inner tags should have appeared in
    //   matching pairs and been skipped, and then we should have skipped the
    //   close tags in question.
    // - All open tags should have matching close tags, which means our inner
    //   loop should always encounter a close tags and drop nesting to 0. If
    //   a close tags is missing, the loop will continue until i >= tokens.length
    ...
    for (var i = 0, token = tokens[0]; token; i += 1, token = tokens[i]) {
      ...
      // top-level open tags
        ...
        pray('not an unmatched top-level close tags', token.charAt(1) !== '/');

        tokens[i] = token.slice(0,-1) + cmdId + '>';

        // skip matching top-level close tags and all tags pairs in between
        var nesting = 1;
        do {
          i += 1, token = tokens[i];
...
//on document ready, mathquill-ify all `<tags class="mathquill-*">latex</tags>`
